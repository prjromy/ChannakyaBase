CREATE function [fin].[FGetAllTellerBranch] (@DegParamId int, @BranchId int)
...
BEGIN
	...
	insert into @Temp
	 ...
	 select e.employeeId, e.EmployeeNo,e.EmployeeName, IsTemp = case when [To] is null then 0 else 1 end, ub.userId,u.UserName  from lg.uservsbranch ub 
...
where mt.designationId = @DegId and status = 1 and BranchId = @BranchId
	...
	select distinct e.EmployeeId,  e.EmployeeName ,e.EmployeeNo, DGName = @DGName, t.userId,t.UserName    from lg.Employees e inner join @Temp t on e.EmployeeId  = t.EmployeeId
	...
	where e.EmployeeId in (select EmployeeId from @Temp where BranchId = @BranchId)
