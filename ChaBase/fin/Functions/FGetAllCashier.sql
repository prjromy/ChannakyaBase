CREATE Function [fin].[FGetAllCashier]()
 returns table 
 as 
 return
 (	select 
	...
	inner join
		...
		(select a.PId,b.PName,PValue from lg.ParamValue a 
		 inner join
		(select * from lg.Param where parentid=2004 and PId=2005) b on a.PId=b.PId) d on d.PValue=cast(c.DesignationId as varchar)) f on e.DesignationId=f.DesignationId
