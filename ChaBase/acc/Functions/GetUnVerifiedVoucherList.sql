CREATE function [acc].[GetUnVerifiedVoucherList](@SDate date,@EDate date,@BranchId int,@VType int,@PostedBy int,@BatchNo varchar(max),@PageNo int,@PageSize int,@CurrentFYID int)
...
as begin
	...
	--select VNId from acc.VoucherNo where VTypeId=@VType and bid=@BatchNo and FYID=@CurrentFYID and companyid=@BranchId
	 set @TotalCount = (select count (v1id) from (select distinct v1.V1Id from acc.voucher1 v1
	...
	select V1Id, FYID, TDate, Narration, CompanyId, PostedBy, UserName, PostedOn, Amount, ApprovedBy, VNo, ApprovedOn, totalCount 
	From (
		...
		v1.ApprovedOn as ApprovedOn,@TotalCount as totalCount, ROW_NUMBER() over (order by TDate) as RowNum
		...
		where ISNULL(DebitAmount,0)!=0
			...
			and vno.FYID=@CurrentFYID
			and v1.PostedBy in( select UserId from lg.[user] where userid=coalesce(case when @PostedBy=0 then null else @PostedBy end,userid))
		group by v1.v1Id,vno.FYID , vt.Prefix,v1.TDate,v1.Narration,vno.CompanyId, v1.PostedBy,u.UserName,v1.PostedOn,v2.DebitAmount,v1.ApprovedBy,v1.vno,bt.prefix,vt.prefix, v1.ApprovedOn
		) x
	where x.RowNum between ((@PageNo-1)*@PageSize)+1 and @PageSize*@PageNo
